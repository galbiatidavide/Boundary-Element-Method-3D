function [G, H_hat, H, alpha] = assembly_quadratic_prova_old(triangles, face_normals, ...
    ...
    for i = 1:n_points
        ...
        for l = 1:6
            ...
            for h = 1:size(quadrature_nodes,1)
                ...
                temp_H_hat = temp_H_hat + basis_functions_eval(h,l)*v_n*weights(h)*area; %*1/4.9;
                temp_G = temp_G + basis_functions_eval(h,l)*v*weights(h)*area; %*1/4.9;
            end
            
            H_hat(i,iglo(l)) = H_hat(i,iglo(l)) + (0.5/4*pi)*temp_H_hat;
            G(i,iglo(l)) = G(i,iglo(l)) + (0.5/4*pi)*temp_G;
