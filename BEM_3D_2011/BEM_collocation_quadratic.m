...
% % Inputs: points (N x 3), sol (N x 1)
% filename = 'point_cloud_solution.vtk';
% fileID = fopen(filename, 'w');
% 
% % Write VTK header
% fprintf(fileID, '# vtk DataFile Version 2.0\n');
% fprintf(fileID, 'Point Cloud with Scalar Solution\n');
...
% 
% % Define vertices (one vertex per point, no connectivity)
% fprintf(fileID, '\nVERTICES %d %d\n', size(nodes, 1), size(nodes, 1) * 2);
% for i = 1:size(nodes, 1)
%     fprintf(fileID, '1 %d\n', i - 1); % Each vertex (1 point per vertex, 0-based indexing)
% end
% 
% % Write solution as scalar data associated with points
% fprintf(fileID, '\nPOINT_DATA %d\n', size(nodes, 1));
% fprintf(fileID, 'SCALARS solution float 1\n');
...
% % Inputs: nodes (N x 3), dn_u_boundary_nodal (N x 1)
% x = nodes(:, 1);
% y = nodes(:, 2);
% z = nodes(:, 3);
% 
% % Visualize the point cloud directly using scatter3
% scatter3(x, y, z, 500, dn_u_boundary_nodal, 'filled');
% colorbar;
% xlabel('X');
% ylabel('Y');
% zlabel('Z');
...
solution = dn_u_boundary_nodal; % Solution at each point

% Define a regular grid over the space
xq = linspace(min(x), max(x), 100); % Grid in x-direction (100 points)
yq = linspace(min(y), max(y), 100); % Grid in y-direction (100 points)
zq = linspace(min(z), max(z), 100); % Grid in z-direction (100 points)

% Create a 3D grid for interpolation
[X, Y, Z] = meshgrid(xq, yq, zq); 

% Perform the interpolation on the 3D grid
V = griddata(x, y, z, solution, X, Y, Z, "linear"); % Cubic interpolation

% Flatten the grid and solution for 3D scatter plotting
x_flat = X(:); % Flatten the X-coordinates
y_flat = Y(:); % Flatten the Y-coordinates
z_flat = Z(:); % Flatten the Z-coordinates
v_flat = V(:); % Flatten the solution values

% Plot the interpolated data as a 3D scatter plot
figure;
scatter3(x_flat, y_flat, z_flat, 10, v_flat, 'filled'); % Color by solution values
colorbar; % Add a colorbar to indicate solution values
colormap jet
xlabel('X');
ylabel('Y');
zlabel('Z');
title('3D Visualization of Interpolated Solution');
axis equal;
