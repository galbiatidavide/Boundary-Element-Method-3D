...
function convergence_test_quadratic(data, norm_type, n_sample_points, which_bem)
...
%% ciclo ok
for k=1:length(data)
    % creating a single data struct with only the current file name
    current_data = data(k);
    %current_data.filename = {data.filename{k}};
    % solving the problem with BEM
    ...
    elseif (strcmp(which_bem, 'quadratic'))
        [u_boundary_nodal, dn_u_boundary_nodal, TR] = ...
            BEM_collocation_quadratic(current_data);
    else
    ...
    for n=1:length(norm_type)
        fprintf('-- CREATING THE STRUCTURES for mesh %d --', n)
        tic
        % relative_errors = zeros(length(norm_type),length(data),);
        % mesh_size = zeros(length(data));
...
for n=1:length(norm_type)
    ...
    title('U_n at boundary', 'Fontsize', 16)
    ...
    legend('relative errors', '1/2 order', '1 order', ...
        '2 order',  'Location', 'Southeast', 'Fontsize', 16)
    title('U at boundary', 'Fontsize', 16)
    ...
    legend('relative errors', '1/2 order', '1 order', ...
        '2 order',  'Location', 'Southeast', 'Fontsize', 16)
    title('U at internal points', 'Fontsize', 16)
end
...
triangles_convergence = zeros(n_elements, 3, 3);
mesh_size = -Inf;
for i = 1:n_elements
