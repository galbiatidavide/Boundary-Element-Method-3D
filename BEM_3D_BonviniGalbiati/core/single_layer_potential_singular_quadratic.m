...
found = find(ismember(node_indexes,[4,5,6]));
...
q_1= @(chi) -0.5*(1./(sin(chi)+cos(chi))) + ...
    2/3*(1./(sin(chi)+cos(chi))).^3.*(1+sin(chi).*cos(chi));
q_2= @(chi) 2/3*(1./(sin(chi)+cos(chi))).^3.*(sin(chi)).^2 - ...
    0.5*(1./(sin(chi)+cos(chi))).^2.*sin(chi);
q_3= @(chi) 2/3*(1./(sin(chi)+cos(chi))).^3.*(cos(chi)).^2 - ...
    0.5*(1./(sin(chi)+cos(chi))).^2.*cos(chi);
q_4= @(chi) 2*(1./(sin(chi)+cos(chi))).^2.*sin(chi) - ...
    4/3*(1./(sin(chi)+cos(chi))).^3.*((sin(chi)).^2+sin(chi).*cos(chi));
q_5= @(chi) 4/3*(1./(sin(chi)+cos(chi))).^3.*sin(chi).*cos(chi);
q_6= @(chi) 2*(1./(sin(chi)+cos(chi))).^2.*cos(chi) - ...
...
values(node_indexes(1)) = coeff * integral(integrand_1,0,pi/2);
values(node_indexes(2)) = coeff * integral(integrand_2,0,pi/2);
values(node_indexes(3)) = coeff * integral(integrand_3,0,pi/2);
values(node_indexes(4)) = coeff * integral(integrand_4,0,pi/2);
values(node_indexes(5)) = coeff * integral(integrand_5,0,pi/2);
values(node_indexes(6)) = coeff * integral(integrand_6,0,pi/2);
